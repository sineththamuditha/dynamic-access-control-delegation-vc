...
import { didWebAgent, getDidFor } from "../../../agents/didWebAgent";
import Papa from "papaparse";
import { attenuatedDelegation } from "./flows/attenuateDelegation";

const StudentSupervisorAttenuated: React.FC = () => {
  ...
  const ATTENUATED_VC_KEY: string = "attenuatedVCKey";
  ...
  const delegateAccessUsingDIDDocument = async () => {
    ...
    const vpForAdc: VerifiablePresentation = await didWebAgent.createVerifiablePresentation({
        presentation: {
            holder: didIdentifiers[STUDENT_DID_IDENTIFIER_KEY].did,
            verifiableCredential: [storedVCs[STUDENT_UNIVERSITY_VC_KEY]]
        },
        proofFormat: 'jwt'
    })

    const attenuatedCredential: VerifiableCredential =
      await attenuatedDelegation(
        vpForAdc,
        didIdentifiers[SUPERVISOR_DID_IDENTIFIER_KEY],
        didIdentifiers[STUDENT_DID_IDENTIFIER_KEY]
      );
    ...
    setStoredVcs((prevVCs) => ({
      ...prevVCs,
      [ATTENUATED_VC_KEY]: attenuatedCredential,
    }));

    setJson(attenuatedCredential);
  ...
  const createVerifiablePresentation = async () => {
    const libraryVerifiablePresentation: VerifiablePresentation =
      await didWebAgent.createVerifiablePresentation({
        presentation: {
            holder: didIdentifiers[STUDENT_DID_IDENTIFIER_KEY].did,
            verifiableCredential: [storedVCs[ATTENUATED_VC_KEY], storedVCs[STUDENT_UNIVERSITY_VC_KEY]]
        },
        proofFormat: 'jwt'
      })
  ...
  const Evaluate: () => Promise<void> = async () => {
    ...
    for (let i = 0; i <= 100; i++) {
      ...
      await didWebAgent.createVerifiablePresentation({
        presentation: {
            holder: didIdentifiers[STUDENT_DID_IDENTIFIER_KEY].did,
            verifiableCredential: [attenuatedCredential, storedVCs[STUDENT_UNIVERSITY_VC_KEY]]
        },
        proofFormat: 'jwt'
      })
    ...
    link.href = url;
    link.setAttribute("download", "supervisorStudentAttenuated.csv");
...
export default StudentSupervisorAttenuated;
